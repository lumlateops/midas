	...
	public static void main(String[] args) throws Exception {
		...
		LinkedList<AccountORM> accountlists = fetchscheduler.accountdao
				.getAllAccounts(); // TODO not scalable. Change it to iterative
									...
									// seperate thread.
		Gson gson = new Gson();
		System.out.println(gson.toJson(accountlists));
		for (AccountORM acc : accountlists) {
			...
			if(emailaddr.isEmpty() || pass.isEmpty()){
				System.out.println("Incomplete dataset for Account id:"+acc.getId());
				continue;
			}
			...
			try {
				...
				fetchscheduler.fetchhistorydao.update(fetchscheduler.fetchorm);
				System.out.println("Error fetching emails for user: "+fetchscheduler.account.getId());
				e.printStackTrace();
			}
		}
